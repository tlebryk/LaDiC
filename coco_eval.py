...
def inference(x, tokenizer, model, time_difference=0):
    with torch.no_grad():
        x_t = torch.randn(
            (x["image"].shape[0], MAX_LENGTH, IN_CHANNEL), device=device
        )  # Gaussian noise (bsz, seqlen, 768)
        ...
        while t > 0:
            ...
            cond_pred = model(
                x["image"].to(device),
                torch.cat([x_t, x_pred], dim=-1).to(device),
            ...
            uncond_pred = model(
                torch.zeros_like(x["image"], device=device),
                torch.cat([x_t, x_pred], dim=-1).to(device),
                torch.ones((x["image"].shape[0], MAX_LENGTH), device=device),
            ...
            ) * cond_pred - CLASSIFIER_FREE_WEIGHT * uncond_pred
            # x_pred = cond_pred
            if t < 600 and t > 300 and flag:
                tmp_out = model.lm_head(
                    model.space_decoder(inputs_embeds=x_pred * X_SIGMA + X_MEAN)[0]
                )
                ...
                x_t = diffuse_t(
                    x_pred, torch.tensor([t], device=device) - STEP, is_test=True
                )
                x_t[torch.arange(x_pred.shape[0])[:, None], to_be_updated_idx] = (
                    gaussian_noise[
                        torch.arange(x_t.shape[0])[:, None], to_be_updated_idx
                    ].clone()
                )
                # indexes1 = nn.functional.softmax(out1, dim=-1).argmax(dim=-1)
        ...
        ans_strs = [
            " ".join(k for k, _ in itertools.groupby(original_str.split()))
            for original_str in ans_strs
        ]
        ans_strs = [original_str.strip(".").strip() + "." for original_str in ans_strs]

    return ans_strs, x.get("img_id", 0)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "image_root": "/mldl/v-yuchiwang/FEDIC/datasets/COCO",
    }
    train_set, val_set, test_set = create_dataset("caption_coco", config)
    val_loader = DataLoader(
        val_set, shuffle=False, batch_size=200, drop_last=False, num_workers=4
    )
    test_loader = DataLoader(
        test_set, shuffle=False, batch_size=200, drop_last=False, num_workers=4
    )
