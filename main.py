...
def visualize_predictions(model, dataloader, num_samples=5):
    ...
    batch = {k: v[:num_samples] if isinstance(v, torch.Tensor) else v for k, v in batch.items()}
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            for j in range(seq_len):
                ...
                distances = torch.norm(token_embeddings - x_0_pred[i, j].unsqueeze(0), dim=1)
        ...
        for i, pred in enumerate(predictions):
            if i < len(batch['text']):
                gt = batch['text'][i] if isinstance(batch['text'], list) else batch['text'][i].item()
                print(f"Image {i+1}:")
...
for epoch in range(start_epoch, EPOCH_NUM):
    ...
    l, x_t_loss, x_1_loss, prob_loss,= validate(model)
